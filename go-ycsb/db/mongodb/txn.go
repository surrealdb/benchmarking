...
func (m *mongoDB) InitThread(ctx context.Context, _ int, _ int) context.Context {
	causalConsistency := false
	sessOpts := options.Session()
	sessOpts.CausalConsistency = &causalConsistency

	sess, err := m.cli.StartSession(sessOpts)
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Error creating MongoDB session: %s", err.Error())
	}

	txnOpts := options.Transaction()
	txnOpts.ReadConcern = readconcern.Snapshot()
	txnOpts.ReadPreference = readpref.Primary()
	txnOpts.WriteConcern = writeconcern.New(writeconcern.WMajority(), writeconcern.J(true), writeconcern.WTimeout(5000))

	ctx = context.WithValue(ctx, MongoDBSession{}, sess)
	ctx = context.WithValue(ctx, MongoDBTransaction{}, txnOpts)

	return ctx
}

func (m *mongoDB) CleanupThread(ctx context.Context) {
	sess := ctx.Value(MongoDBSession{}).(mongo.Session)
...
func (m *mongoDB) Read(ctx context.Context, table string, key string, fields []string) (map[string][]byte, error) {
	sess := ctx.Value(MongoDBSession{}).(mongo.Session)
	txnOpts := ctx.Value(MongoDBTransaction{}).(*options.TransactionOptions)
...
func (m *mongoDB) Update(ctx context.Context, table string, key string, values map[string][]byte) error {
	...
	_, err := sess.WithTransaction(ctx, func(ctx mongo.SessionContext) (interface{}, error) {
		...
		if res.MatchedCount != 1 {
			return nil, fmt.Errorf("Update error: %s not found", key)
		}
		return nil, nil
	}, txnOpts)
...
func (m *mongoDB) Delete(ctx context.Context, table string, key string) error {
	...
	_, err := sess.WithTransaction(ctx, func(ctx mongo.SessionContext) (interface{}, error) {
		...
		if res.DeletedCount != 1 {
			return nil, fmt.Errorf("Delete error: %s not found", key)
		}
		return nil, nil
	}, txnOpts)
...
func init() {
	ycsb.RegisterDBCreator("mongodb_txn", mongodbCreator{})
}
